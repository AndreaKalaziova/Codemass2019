...
//zpusob jakym bude canvas vykreslovat objekty - 2D, pomoci funkce getContext urcime zpusob vykreslovani
let ctx = canvas.getContext("2d");
let width = 1000;
let height = 600;
let friction = 0.8; // treni, aby se pohyboval plynule
let gravity = 0.3; // gravitace, aby nevyskocil jako raketa 
let keys = [];
let platforms = [];
let presents = [];
let gingerbreads = [];
...
// definice player-a = pridani obrazku
let player = {
    x: width / 2,
    y: height - 30,
    width: 30,
...
let hero = new Image();
hero.src = "pics/elf_left.png";

let platformImage = new Image();
platformImage.src = "pics/platform.png";

let spike = new Image();
spike.src = "pics/spike.png";

let present = new Image();
present.src = "pics/present.png";

let gingerbread = new Image();
gingerbread.src = "pics/gingerbread.png";

let portal = new Image();
portal.src = "pics/portal.png";

canvas.width = width;
canvas.height = height;

//funkce na vyherny portal
function createExit() {
    // exit
    exit.push({
        x: 850,
...
//funkce vykresleni(hero=obrazek, umisteni x, umisteni y, sirka, vyska)
...
//funkce, která kontroluje sběr dárečků a preníčků
function colItems() {
...
// funkce, ktere se stara o pohyb
function movement() {
    if (keys[39]) {
        if (player.velX < player.speed) {
            player.velX++;
        }
        hero.src = "pics/elf_right.png";
    }// podminka pro stisk prave sipky, pokud je stisknuta-navysi se rychlost
    ...
    player.velX *= friction; // usmerni pohyb
    player.velY += gravity;

    player.x += player.velX;
    player.y += player.velY;
    // meni se pozice/hybe se - aby akce probehla, upravi f.update hore
    /* aby nemizel z pole hore/dole 
    ...
    //po kazdem snimku se predosli zmaze

    player.grounded = false;
}

//pohyb elfa pomoci sipek
// body-> umisteni pro cely dokument
//keydown - stisk nebo drzeni libovolne klavesy
// keyup se zakativuje po spusteni v.u. klavesy
...
// zustane na konci
// posluchac udalosti - Objekt(window-okno prohlizece), ke kteremu se vaze(udalost, na kterou ceka->load=dokud nebude vsechno nactene, funkce, ktera se spusti(po load-u))
